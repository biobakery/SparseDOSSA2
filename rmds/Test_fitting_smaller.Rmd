...
tb_simulation <- tidyr::crossing(
  ...
  n = c(1000),
  n_r = c(10000),
  lambda = exp(seq(log(1e-3), log(1), length.out = 5)),
  maxit = 30
) %>% 
...
for(i_job in seq_len(njobs)) {
  l_result[[i_job]] <- one_job(i_job)
}
save(l_result, file = paste0(dir_output, "l_result.RData"))
```

```{r visualize}
l_result <- list()
for(i_job in 1:9) {
  load(paste0(dir_output, "params_x_", i_job, ".RData"))
  l_result[[i_job]] <- params_x
}

tb_simulation <- tb_simulation %>% 
  dplyr::mutate(i_job = seq_len(nrow(tb_simulation)))
tb_compare <- l_result %>% 
  purrr::imap_dfr(
    ~ .x %>% purrr::imap_dfr(
      ~ .x %>% purrr::imap_dfr(
        ~ tibble::tibble(value = as.vector(.x),
                         param = .y)
      ) %>% 
        dplyr::mutate(class = .y)
    ) %>% 
      dplyr::mutate(i_job = .y)
  ) %>% 
  dplyr::left_join(tb_simulation, by = "i_job")

l_plots <- unique(tb_compare$param) %>% 
  purrr::map(function(i_param) {
    tb_compare %>% 
      dplyr::filter(param == i_param) %>% 
      ggplot(aes(x = as.character(log(lambda)), 
                  y = value,
                  color = class)) +
      geom_boxplot() +
      facet_grid(.~pi0)
...
for(i in seq_along(l_plots)) {
  ggsave(filename = paste0(dir_output, "fig_", unique(tb_compare$param)[i], ".pdf"),
         plot = l_plots[[i]],
         width = 8, height = 4)
}
```