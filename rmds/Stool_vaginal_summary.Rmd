...
tb_result <- tb_result %>% 
  ...
  dplyr::mutate(log10_lambda = log10(lambdas) %>% 
                  ...
                  forcats::as_factor()) %>% 
  dplyr::group_by(i_job) %>% 
  dplyr::mutate(sparsity_Omega = fit_EM[[1]]$fit$Omega %>% 
                  SparseDOSSA2:::lower_tri() %>% {mean(. == 0)},
                sparsity_Sigma = fit_EM[[1]]$fit$Sigma %>% 
                  SparseDOSSA2:::lower_tri() %>% {mean(. == 0)},
                logLik_CV = mean(setdiff(
                  fit_EM[[1]]$logLik_CV[, 1] + {
                  if(dataset == "vaginal") {
                    apply(x_samples_vaginal, 
                          1, 
                          function(x) 
                            - sum(log(x[x > 0])))
                  } else {
                    apply(x_samples_stool, 
                          1, 
                          function(x) 
                            - sum(log(x[x > 0])))
                  }}, -Inf)),
                logLik_CV_sd = sd(setdiff(
                  fit_EM[[1]]$logLik_CV[, 1] + {
                  if(dataset == "vaginal") {
                    apply(x_samples_vaginal, 
                          1, 
