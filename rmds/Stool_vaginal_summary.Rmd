...
knitr::opts_knit$set(root.dir = "/n/hutlab11_nobackup/users/syma/sparsedossa_update/")
library(magrittr)
library(ggplot2)
dir_output <- "/n/hutlab11_nobackup/users/syma/sparsedossa_update/results/Stool_vaginal/"
...
tb_simulation <- tb_simulation %>% 
  ...
  dplyr::mutate(params = 
                  {
                    load(paste0(dir_output, "fit_", i, ".RData"))
                    fit_EM$ll_params[[length(fit_EM$ll_params)]] %>% 
                      list()
                  }) %>% 
  dplyr::ungroup() %>% 
...
tb_simulation <- tb_simulation %>% 
  dplyr::mutate(l = params %>% 
                  purrr::map_dbl("l"),
                sparsity_Omega = params %>% 
                  purrr::map_dbl(
                    ~ mean(.x$Omega[lower.tri(.x$Omega)] == 0)
                  ),
                sparsity_Sigma = params %>% 
                  purrr::map_dbl(
                    ~ mean(.x$Omega[lower.tri(.x$Sigma)] == 0)
                  ))
tb_simulation %>%
  ggplot(aes(x = log10(lambda), y = l)) +
